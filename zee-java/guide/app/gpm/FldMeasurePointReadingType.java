...
import psdi.app.meter.FldMeterReadingType;
import psdi.mbo.MboRemote;
import psdi.mbo.MboSetRemote;
import psdi.mbo.MboValue;
import psdi.util.MXException;

public class FldMeasurePointReadingType extends FldMeterReadingType {

	public FldMeasurePointReadingType(MboValue mbv) throws MXException {
		super(mbv);
	}
	...
	public void init() throws MXException, RemoteException {
		...
		if ("OIL".equalsIgnoreCase(pmtype)) {
			if (owner != null) {
				MboSetRemote lineSet = owner.getMboSet("OILMATUSE");
				if (!lineSet.isEmpty() && lineSet.count() > 0) {
					mbo.setFieldFlag("READINGTYPE", 7L, true);
				} else {
					mbo.setFieldFlag("READINGTYPE", 7L, false);
				}
			}
		} else {
			MboSetRemote lineSet = mbo.getMboSet("UDMEASUREMENT");
			if (!lineSet.isEmpty() && lineSet.count() > 0) {
				mbo.setFieldFlag("READINGTYPE", 7L, true);
			} else {
				mbo.setFieldFlag("READINGTYPE", 7L, false);
