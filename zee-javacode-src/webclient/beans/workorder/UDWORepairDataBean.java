 ...
 *@modify:
 */
public class UDWORepairDataBean extends DataBean{
	@Override
	...
	public synchronized int execute() throws MXException, RemoteException {
		...
		if(wo != null) {
			...
			String tobin = "ABC";
			double linecost = Double.parseDouble(getString("pocost").replace(",", "")) * Double.parseDouble(getString("quantity"));
			...
			double pocost = Double.parseDouble(getString("pocost").replace(",", ""));//维修PO的总成本（服务+物资）
			...
			if (existInvcost == false) {
				String sql3 = "insert into invcost(itemnum, location, condrate, itemsetid, siteid, stdcost, avgcost, lastcost, controlacc, orgid, invcostid)"
						+"values"
						+"('"+itemnum+"','"+location+"','100','"+itemsetid+"','"+siteid+"','"+pocost+"','"+pocost+"','"+pocost+"','COSCO', '"+orgid+"',invcostSEQ.NEXTVAL)";
				try {
					exeSQL(sql3);
				} catch (SQLException e) {
					e.printStackTrace();
				...
				updateInvcost(itemnum,location,pocost,quantity);
			}
			...
			String sql5 = "insert into matrectrans(itemnum,tostoreloc,transdate,actualdate,quantity,receivedunit,issuetype,unitcost,actualcost,ponum,rejectqty,conversion,enterby,outside,polinenum,issue,requestedby,totalcurbal,tobin,gldebitacct,linecost,financialperiod,currencycode,exchangerate,currencyunitcost,currencylinecost,description,loadedcost,tax1code,tax1,tax2,tax3,tax4,tax5,prorated,proratecost,status,curbal,exchangerate2,matrectransid,orgid,siteid,costinfo,enteredastask,fromsiteid,linetype,itemsetid,langcode,inspectedqty,positeid,hasld,porevisionnum,consignment,udztype)"
					+"values"
					+"('"+itemnum+"','"+location+"',sysdate,sysdate,'"+quantity+"','"+receivedunit+"','"+issuetype+"','"+pocost+"','"+pocost+"','"+ponum+"','0','1','"+enterby+"','0','"+polinenum+"','0','"+enterby+"','0','"+tobin+"','COSCO','"+linecost+"','COSCO','EUR','1','"+pocost+"','"+linecost+"','"+description+"','"+linecost+"','"+tax1code+"','0','0','0','0','0','0','0','"+status+"','0','1',matrectransSEQ.NEXTVAL,'"+orgid+"','"+siteid+"','1','0','"+fromsiteid+"','"+linetype+"','"+itemsetid+"','"+langcode+"','0','"+positeid+"','0','0','0','0')";
			try {
				exeSQL(sql5);
			} catch (SQLException e) {
				e.printStackTrace();
			...
			wo.setValue("udworepairitemnum", itemnum, 11L);
			wo.setValue("udworepairtimes", "Y", 11L);
	...
	private void updateInvcost(String itemnum,String location,double pocost,double quantity) throws MXException,RemoteException {
		...
		if (!invcostSet.isEmpty() && invcostSet.count() > 0) {
			...
			if (!invbalancesSet.isEmpty() && invbalancesSet.count() > 0) {
				...
				double newavgcost = (avgcost * (curbal-quantity) + pocost * quantity) / ((curbal-quantity) + quantity);
